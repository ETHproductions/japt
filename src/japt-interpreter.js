...
df(Array,'y',function(x,y){if(id(x)){var z=this.y(),a=z.m(function(c){return c.f(id)}).m(x,y);if(a.every(function(q){return typeof q==="string"||q instanceof Array}))z=z.m(function(c,i){var j=0;return c.map(function(){return a[i][j++]})}).y();else z=a;return z}var t="string"==typeof this[0],n=t?this.map(function(t){return t.split("")}):this,x,y,z=n.reduce(function(p,q){return Math.max(p,q.length)},0),a=[];for(y=0;y<z;y++)a[y]=t?Array(n.length).fill(" "):[];for(y=0;y<n.length;y++)for(x=0;x<n[y].length;x++)a[x][y]=n[y][x];return t?a.map(function(r){var i=0;return r.join("")}):a});
